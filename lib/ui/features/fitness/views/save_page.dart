...
class _SavedPageState extends State<SavedPage> {
  ...
  // Cached references so dispose() can remove listeners without touching context.
  NutritionViewModel? _nutritionVm;
  FitnessViewModel?   _fitnessVm;

  @override
  void didChangeDependencies() {
    super.didChangeDependencies();
    if (_nutritionVm == null) {
      _nutritionVm = context.read<NutritionViewModel>();
      _fitnessVm   = context.read<FitnessViewModel>();
      _nutritionVm!.loadAllAnalyses();
      _nutritionVm!.addListener(_loadSavedData);
      _fitnessVm!.addListener(_loadSavedData);
    }
  }

  @override
  void dispose() {
    _nutritionVm?.removeListener(_loadSavedData);
    _fitnessVm?.removeListener(_loadSavedData);
    super.dispose();
  }
  ...
  void _loadSavedData() {
    ...
    for (var analysis in _nutritionVm!.analyses) {
      items.add(SavedItem(
    ...
    for (var plan in _fitnessVm!.plans) {
      items.add(SavedItem(
