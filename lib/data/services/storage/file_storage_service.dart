...
abstract class FileStorageDataSource {
  /// Copies [sourcePath] into app storage and returns a path relative to the
  /// documents directory (e.g. "fitness_images/123_photo.jpg"). Relative
  /// paths stay resolvable even if the documents dir's absolute prefix
  /// changes between app sessions (see [ImagePathResolver]).
...
class FileStorageDataSourceImpl implements FileStorageDataSource {
  ...
  Future<String> saveImageFile(String sourcePath) async {
    ...
    return '$_imagesDirectory/$uniqueFileName';
  }

  @override
  Future<void> deleteImageFile(String imagePath) async {
    final resolved = await ImagePathResolver.resolve(imagePath);
  ...
  Future<bool> imageFileExists(String imagePath) async {
    return await ImagePathResolver.resolve(imagePath) != null;
  }
}
