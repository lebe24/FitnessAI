...
class FitnessBloc extends Bloc<FitnessEvent, FitnessState> {
  ...
  Future<void> _onLoadFitnessPlans(
    ...
    try {
      ...
      debugPrint('FitnessBloc: Loaded ${plans.length} fitness plans');
      final workoutMappings = _mapPlansToDates(plans);
      debugPrint('FitnessBloc: Created ${workoutMappings.length} workout mappings');
      ...
      debugPrint('FitnessBloc: Error loading fitness plans: $e');
      debugPrint('FitnessBloc: Stack trace: $stackTrace');
  ...
  Future<void> _onWorkoutCompleted(
    ...
    if (state is FitnessLoaded) {
      ...
      if (currentState.completedDates.contains(normalizedDate)) {
        debugPrint('Workout already completed for date: $normalizedDate');
        return; // Already completed, don't increment streak again
      }

      // Add to completed dates
      final updatedCompletedDates = <DateTime>{...currentState.completedDates, normalizedDate};
      ...
      debugPrint('Workout completed. New streak: $newStreak');
  ...
  Future<Set<DateTime>> _loadCompletedDates() async {
    try {
      ...
      if (datesList is List<dynamic>) {
        final dates = datesList.map((dateStr) {
        ...
        debugPrint('Loaded ${dates.length} completed workout dates');
        return dates;
      }
      return <DateTime>{};
    } catch (e) {
  ...
  Future<void> _saveCompletedDates(Set<DateTime> dates) async {
    try {
      ...
      await box.flush(); // Ensure data is persisted
      debugPrint('Saved ${dates.length} completed workout dates');
    } catch (e) {
      debugPrint('Error saving completed dates: $e');
    }
  }
  ...
  Future<int> _loadStreak() async {
    try {
      ...
      final streak = value is int ? value : 0;
      debugPrint('Loaded streak: $streak');
      return streak;
    } catch (e) {
  ...
  Future<void> _saveStreak(int streak) async {
    try {
      ...
      await box.flush(); // Ensure data is persisted
      debugPrint('Saved streak: $streak');
    } catch (e) {
